...
mod tests {
    ...
    fn setup_routes_dir() -> std::io::Result<(TempDir, PathBuf)> {
        ...
        // Static route
        File::create(routes.join("route.ts"))?.write_all(b"export default {}")?;

        // Users routes
        let users = routes.join("users");
        fs::create_dir(&users)?;
        File::create(users.join("route.get.ts"))?.write_all(b"export default {}")?;
        File::create(users.join("route.post.ts"))?.write_all(b"export default {}")?;

        // Dynamic route
        let user_id = users.join("[id]");
        fs::create_dir(&user_id)?;
        File::create(user_id.join("route.get.ts"))?.write_all(b"export default {}")?;
        File::create(user_id.join("route.delete.ts"))?.write_all(b"export default {}")?;
        ...
        File::create(health.join("route.ts"))?.write_all(b"export default {}")?;
...
mod regex_validation_tests {
    ...
    fn setup_specific_routes() -> std::io::Result<(TempDir, PathBuf)> {
        let cwd = std::env::current_dir()?;
        let temp_dir = TempDir::new_in(&cwd)?;
        let root = temp_dir.path().to_path_buf();
        ...
        fs::create_dir(&health)?;
        File::create(health.join("route.get.ts"))?.write_all(b"export default {}")?;

        // /hello route
        let hello = routes.join("hello");
        fs::create_dir(&hello)?;
        File::create(hello.join("route.get.ts"))?.write_all(b"export default {}")?;
